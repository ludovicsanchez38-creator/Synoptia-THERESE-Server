...
Gestion centralisée des erreurs avec messages clairs.

US-ERR-01: Messages d'erreur clairs si API down
US-ERR-02: Retry automatique en cas de timeout
US-ERR-03: Mode dégradé si Qdrant indisponible
"""
...
# Messages d'erreur en français (US-ERR-01)
ERROR_MESSAGES = {
    ErrorCode.API_UNREACHABLE: "Le service {service} est temporairement indisponible. Vérifiez votre connexion internet.",
    ErrorCode.API_TIMEOUT: "Le service {service} met trop de temps à répondre. Nouvelle tentative en cours...",
    ErrorCode.API_RATE_LIMITED: "Trop de requêtes envoyées. Attendez quelques instants avant de réessayer.",
    ErrorCode.API_AUTH_FAILED: "Clé API {provider} invalide ou expirée. Vérifiez vos paramètres.",
    ErrorCode.API_SERVER_ERROR: "Erreur serveur chez {provider}. L'équipe technique est probablement au courant.",
    ErrorCode.LLM_PROVIDER_UNAVAILABLE: "Le modèle {model} n'est pas disponible. Essayez un autre provider.",
    ...
    ErrorCode.LLM_GENERATION_FAILED: "La génération de réponse a échoué. Réessayez dans quelques instants.",
    ErrorCode.DB_CONNECTION_FAILED: "Impossible de se connecter à la base de données locale.",
    ErrorCode.DB_QUERY_FAILED: "Erreur lors de l'accès aux données. Redémarrez l'application si le problème persiste.",
    ErrorCode.QDRANT_UNAVAILABLE: "La recherche sémantique est temporairement indisponible. Le chat fonctionne sans mémoire.",
    ErrorCode.QDRANT_SEARCH_FAILED: "La recherche dans la mémoire a échoué. Résultat partiel possible.",
    ErrorCode.MCP_SERVER_FAILED: "Le serveur MCP {server} n'a pas pu démarrer. Vérifiez la configuration.",
    ErrorCode.MCP_TOOL_FAILED: "L'outil {tool} a rencontré une erreur: {error}",
...
class TheresError(Exception):
    """Exception personnalisée THERESE avec message utilisateur."""
...
async def retry_with_backoff(
    ...
    Args:
        ...
        max_delay: Délai max en secondes
    ...
    Raises:
        TheresError: Si toutes les tentatives échouent
    """
    last_exception = None
...
def with_retry(
    ...
    """Decorator pour ajouter du retry à une fonction async."""
...
class ServiceStatus:
    ...
    def should_check(self, service: str) -> bool:
        """Vérifie si on doit retester le service."""
        import time
...
async def with_graceful_degradation(
    ...
    """
    Exécute une fonction avec fallback gracieux.

    Si la fonction principale échoue, exécute le fallback ou retourne default_value.
    """
